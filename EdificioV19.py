...
class Oficina(Edificio):
        ...
        def __str__(self):
            aux=super().__str__(self)
            
            return aux+", vestibulo:" + str(self.aforo)
        ...
        #Metodo que hace que las personas realizen su comportamiento programado     
        def abrirCerrar (self, hora, simulador):
           
            if self.edificioAbierto[hora]==True and self.edificioAbierto.iloc[hora-1] == False:
                simulador.desbloquearservicio(simulador.ciudadOficinas[self.numeroEdificio])
            elif self.edificioAbierto[hora]==False and self.edificioAbierto.iloc[hora-1] ==True:                       
                for u in reversed(self.vestibulo):
                    u.volver_a_casa(hora, simulador)
                if self in simulador.serviciosDisponibles:
                    simulador.bloquearservicio(simulador.ciudadOficinas[self.numeroEdificio])    
