...
class Persona:
        def __init__(self, idpersona,dtrEdad):           
            ...
            self.idVivienda = (-1, -1) #MODIFICAR
            self.idOficina = (-1, -1)  #MODIFICAR
        ...
        def __str__(self):
            return "ID: "+str(self.idpersona) + ", Estado: " + str(self.estado) + ", DNI: "+str(self.dni)+", edad:"+str(self.edad)+", Vivienda y piso:" +str(self.idVivienda[0])+ ","  +str(self.idVivienda[1]) +", Oficina y piso:" +str(self.idOficina[0]) + "," + str(self.idOficina[1])
        ...
        def simular_ia(self, hora, simulador):
            ...
            if(!self.esta_sano()):
                 if(self.lugarActual(2) != 0):
                    simulador.irse(self, self.idOficina)
                    simulador.meterse(self, self.idVivienda)
                    self.lugarActual(2) = 0 
                else:
                    pass 
            else:
                if(self.lugarActual(2) == 0 or self.lugarActual(2) == 2):
                     if(self.horarios.iloc[hora] == True): #en caso de que tenga que ir al trabajo...                   
                         print("Debo ir al trabajo")
                         self.lugarActual(2) = 1 #se actualiza el valor
                        simulador.irse(self, self.idVivienda)
                        simulador.meterse(self, self.idOficina)
                    else: #sino...
                        pass
                #Se comprueba    
                elif(self.lugarActual(2) == 1):
                     if(self.horarios.iloc[hora] == False): 
                        self.lugarActual(2) = 0 #se actualiza el valor
                        simulador.irse(self, self.idOficina)
                        simulador.meterse(self, self.idVivienda)
                    else:
                        pass 
